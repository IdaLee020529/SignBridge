...
const SignInfo = () => {
  ...
  async function getSignInfoforPublic() {
    ...
    setRejectedValue(
      dataset.data.filter((data: any) => data.status_SE === "Cancelled").length
    );
  }
    ...
    <>
      ...
      {publicTableData.length === 0 && (roleAccess === "public" || roleAccess === "signexpert") && (
        <div className={style.signTextTableContainer}>
          <div>
            <p>
              No data available. Submit the{" "}
              <a href="/dataset-collection" className={style.linkRedirect}>
                <span>dataset collection form</span>
              </a>{" "}
              to share your words.
            </p>
          </div>
